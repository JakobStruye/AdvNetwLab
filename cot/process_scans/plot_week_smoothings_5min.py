...
if __name__ == '__main__':
    ...
    for i in range(len(daylist)):
        ...
        if fix_endpoints:
            for (hour, minute) in itertools.product(range(20,24), range(first_minute, 60, 10)):
                max_point = process_point(prev_day, hour, minute)
                maxes.append(max_point)

        for (hour, minute) in itertools.product(range(24), range(first_minute, 60, 10)):
            max_point = process_point(day, hour, minute)
            maxes.append(max_point)

        if fix_endpoints:
            for (hour, minute) in itertools.product(range(0,4), range(first_minute, 60, 10)):
                max_point = process_point(next_day, hour, minute)
                maxes.append(max_point)
        daylist[i].append(maxes)

    x = np.linspace(0, 24, 720) if not fix_endpoints else np.linspace(-4, 28, 192)
    ...
    plt.savefig("./images/yolo/"+sys.argv[8]+"_"+day[0]+"_"+"chan"+str(channel_number)+"_image.png", bbox_inches='tight')
    # plt.savefig("./images/yolo/"+sys.argv[8]+"_"+day[0]+"_"+"chan"+str(channel_number)+"_image.pdf", bbox_inches='tight')
