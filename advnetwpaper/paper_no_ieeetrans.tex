...
Playing around with some parameters, we reduced the time between scans from 10 minutes to 2 minutes and discard the 100 largest readings for every sample (when looking at all the channels). This limit of 2 minutes was the minimum we could achieve while guaranteeing a scan would succeed and the number of samples to discard was chosen based on some experimentation.
Using an adapted version of Simon Wünderlich's fft$\_$eval code, Python's Matplotlib library and Savitzky-Golay (source? TODO) filtering we graphically represent our data points per node. For CoT-node12 (location : UAntwerpen campus Middelheim, indoors), looking at the 2.4 GHz band on every channel from monday May 15th to monday May 22nd, we got the following graph:\\
\includegraphics[scale=0.7]{images/window_361_2.4/2017-05-22cot-node12-student_image.pdf}\\
Each line/color represents a day, using this representation we tried to find recurring patterns in the measurements for every node separately. When looking at the complete set of channels, patterns were not as clear as we'd hoped.\\
Given this and the fact that a large amount of static on one channel would make it seem like the entire spectrum is full we switched to drawing the graphs per non-overlapping channel and observing the results in that way. This lead to following graph on which we can clearly see that on weekdays there is substantially more traffic during the daytime than on the weekend (which is as expected for a university campus):\\
\includegraphics[scale=0.7]{images/2017-05-22cot-node12-student_image.pdf}\\

\section{Findings}
\subsection{Evolution throughout the day}
\subsection{Evolution throughout the week}
\subsection{Recurring patterns between days} 
On the different nodes we had varying degrees of success in finding recurring patterns. For this report we've only included the nodes on which we did find such patterns (and we have included the non pattern-y graphs in the zip file that is included // they are available on demand? TODO) 
\subsection{Calm and busy channels}
1 busy, 6, 11 calm
\subsection{2.4 GHz vs 5 GHz}
\subsection{Validation}
scans met / zonder sturende laptop - stukje had gij al
dagelijkse patronen
outside / inside t zelfde +-
