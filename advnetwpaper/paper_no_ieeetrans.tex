...
At first we measured the spectrum on every channel in both the 2.4GHz and the 5GHz band every 10 minutes. Looking at interpretations of the resulting data it was clear that a shorter time between scans would be useful because recurring spikes were missed on some days while they definitely there and also quite interesting.
\\< images waarop da duidelijk is >\\
We then reduced the time between scans from 10 minutes to 2 minutes, which was the minimum we could achieve while guaranteeing that scans would be succesful.
Using an adapted version of Simon Wünderlich's fft_eval code, GNUplot and Savitzky-Golay (source? TODO) filtering we graphically represent our data points per node.
<tussen maandag 15 tem zondag 21 mei> 
